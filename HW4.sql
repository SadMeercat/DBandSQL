...
select mark, count(distinct color) as color from auto where mark = "BMW" or mark = "LADA" group by mark;

	#2. Вывести на экран марку авто и количество AUTO не этой марки
select mark, (select count(mark) from auto b where a.mark <> b.mark) as "other" from auto a group by mark;
...
select id from (select id, data from test_a union all select id, null from test_b) c group by id having count(id) = 1;
